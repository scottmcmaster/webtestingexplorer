 ...
 * specific CSS class name. The class name must be an exact match.
 * 
 * @author scott.d.mcmaster@gmail.com (Scott McMaster)
 */
public class ClassIndexWebElementIdentifier extends IndexWebElementIdentifier {

    private final static Logger LOGGER = Logger.getLogger(ClassIndexWebElementIdentifier.class.getName());

	private String className;
	
	public ClassIndexWebElementIdentifier(String frameIdentifier,
			String className, int index, IndexBasis basis) {
	...
	public WebElementWrapper findElement(WebDriverWrapper driver) {
		...
		for (WebElementWithIdentifier element : allElements) {
			if (className.equals(element.getElement().getAttribute("class")) &&
					currentIndex == index) {
